...
> A distributed system is a system that has some set of nodes, that are connected to each other via a network. The goal of a distributed system is to make the network work as if it were a single node. In Bitcoin, to the outside world it would seem as if the network is working as a single node to compute blocks.
